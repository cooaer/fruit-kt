...
- 核心模块 `:fruit`
    ...
    - 仅保留平台无关核心能力：注解、`Fruit` 入口、`PickAdapter`/`PickAdapterFactory`、基础类型读取、HTML
      解析核心逻辑。
    - 移除核心模块对 Ktor 的直接依赖与 API 暴露。

- 扩展与工具模块
    - `:fruit-ksp` 保持 JVM/KSP processor 模块定位，只负责生成 `*PickAdapter` 和注册函数。
...
- 功能验收
    ...
    - `:fruit-ksp` 有生成回归用例，验证 `@Pulp`/`@Pick` 仍能生成可编译适配器与注册代码。
